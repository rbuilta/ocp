...
. You will then see a list of images - *NOTE - the ocp-release:4.6.x* image that was previously not in the original drop down list
  image:images/VisualWebTerminal-View-Images.png[]
. To make the image visible to be able to deploy to Azure Gov the yaml needs to be edited to allow it to be visible - do this by the following:
  image:images/Edit-ClusterImageSet-visable-false.png[]
