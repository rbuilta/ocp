...
**As of this writing the 'destroy' cluster capabilities are not working for Azure Gov from ACM - you will need to destroy the cluster from ACM as well as go into Azure Gov UI and destroy the cluster**
**Also as far as the cluster showing up in ACM - it keeps a status of "importing cluster" even if the cluster is up and running**
